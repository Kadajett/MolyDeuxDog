...
local function new()  
	...
	local extraGroup = display.newGroup()
	mainGroup:insert(firstGroup)
	mainGroup:insert(objectGroup)
	mainGroup:insert(enemyGroup)
	mainGroup:insert(extraGroup) 

	--Sounds
	local coinSound = audio.loadSound("sounds/Collect.mp3")   --Load sounds
	local overSound = audio.loadSound("sounds/Defeat2.mp3")
	local winSound = audio.loadSound("sounds/LevelClear.mp3")
	...
	local onCollision --This is done so runtime listeners can be easily removed/added 
	...
	local scoreText = display.newText(extraGroup, "Score: "..score,0,0,"Arial",17)
	...
	local function gameOver()
		...
		local function nowEnd()
			...
			timer.performWithDelay(1400, function() director:changeScene("gameOver", "moveFromRight") end, 1)
		end
	...
	local function gameWon()
		...
		--Delay the level change
		timer.performWithDelay(1800, function() director:changeScene("gameWon", "moveFromRight") end, 1)
	end
	...
	-------------------------------------------
	-- ***Clean Function***
	-- Not actually used in this template but i did it anyway
	--------------------------------------------
	local function clean()
		--Remove timers and transitions first.
		if moveTimer then timer.cancel(moveTimer); moveTimer = nil; end
		...
		--This isnt need as they were inserted into the mainGroup.
		--But i do it just out of habit...
		if firstGroup then display.remove( firstGroup ); firstGroup = nil; end
		if objectGroup then display.remove( objectGroup ); objectGroup = nil; end
		if enemyGroup then display.remove( enemyGroup ); enemyGroup = nil; end
		if extraGroup then display.remove( extraGroup ); extraGroup = nil; end
	end
	M.clean = clean
